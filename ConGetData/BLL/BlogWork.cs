...
{
    public class BlogWork : ISpiderData
    {
        ...
        public void RunWork(CrawlTarget target)
        {
           ...
           // string xmlPath = @"H:\2014Year\XmlTemp\博客\" + target.Tid + ".xml";
            ...
            if (result>0)
            {
                File.AppendAllText("site.csv", string.Format("\"{0}\",\"{1}\",\"3\"\n", target.SiteId, target.Tid));
        ...
        public int InsertSiteData(DataModel model)
        {
            ...
            try
            {
                ...
                result = HelperSQL.ExecNonQuery("usp_Spider_Insert_Blog", parms);

            }
            catch (Exception ex)
            {

                LogBLL.Log("usp_Spider_Insert_Blog", parms);
