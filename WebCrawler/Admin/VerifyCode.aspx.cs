...
namespace WebCrawler.Admin
{
       ...
       
    {
        ...
        private static Color BackColor = Color.FromArgb(196,189,177);
        ...
        {
            //Guid.NewGuid().ToString().Substring(0, 4); //GUID取4个字符
            ...
            for (int i = 0; i < 4; i++)
            {
                double m = Math.Round(r.NextDouble() * 10);
                code += m<=1?m:m-1;
            }
